...
import { ArrowLeft, Mail, Phone, Trophy, Star, MoreVertical, Settings, Trash2, Loader2, AlertCircle } from 'lucide-react';
...
export function TeamDetailsScreen({ onBack, teamData, onNavigate }: TeamDetailsScreenProps) {
  ...
  const handleRemoveMember = async (playerId: string) => {
    ...
    const ok = typeof window !== 'undefined' ? window.confirm(`¿Estás seguro de que quieres eliminar a ${playerName} del equipo?`) : true;
    if (!ok) return;
    try {
      const teamRef = doc(db, 'teams', team.id);
      ...
      try {
        ...
        if (chatSnap.exists()) {
          ...
          await addDoc(messagesRef, {
            ...
            createdAt: serverTimestamp(),
            system: true,
            user: { _id: currentUser?.uid, name: 'Sistema' }
          });
                  ...
                  <Card key={player.id} className="border-none shadow-md hover:shadow-lg transition-shadow">
                    <CardContent className="p-4 flex items-center gap-4">
                      <Avatar className="h-12 w-12 border border-gray-100">
                        ...
                        <AvatarFallback className="bg-gray-100 text-gray-500">{playerName !== 'Usuario sin nombre' ? playerName.charAt(0).toUpperCase() : ''}</AvatarFallback>
                      </Avatar>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <h3 className="text-[#172c44] font-bold text-base truncate">{playerName}</h3>
                          {player.id === team.captainId && (<Badge className="bg-[#f4b400] text-[#172c44] text-[10px] px-1.5 py-0.5">CAPITÁN</Badge>)}
                          {player.id === currentUser?.uid && (<Badge className="bg-[#172c44] text:white text-[10px] px-1.5 py-0.5">TÚ</Badge>)}
                        </div>
                        <div className="text-xs text-[#00a884] font-medium mt-0.5 mb-1">{player.favoritePosition ? player.favoritePosition : 'Sin posición específica'}</div>
                        <div className="flex flex-col gap-0.5">
                          <div className="flex items-center gap-1.5 text-gray-600 text-sm"><Mail size={14} className="text-[#00a884]" /><span className="truncate">{player.email || 'Sin correo'}</span></div>
                          <div className="flex items-center gap-1.5 text-gray-600 text-sm"><Phone size={14} className="text-[#00a884]" /><span className="truncate">{player.phoneNumber || player.phone || player.telefono || 'Sin teléfono'}</span></div>
                        </div>
                      </div>
                      {isCaptain && player.id !== currentUser?.uid && (
                        <button onClick={() => handleRemoveMember(player.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-full transition-all" title="Eliminar miembro del equipo">
                          <Trash2 size={20} />
                        </button>
                      )}
                    </CardContent>
              ...
              <CardContent>
                ...
                <div className="mt-6 space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-bold text-[#172c44]">{team.stats.matchesPlayed > 0 ? Math.round((team.stats.wins / team.stats.matchesPlayed) * 100) : 0}%</span>
                  </div>
                  <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden">
                    <div className="bg-[#00a884] h-3 rounded-full transition-all duration-1000" style={{width: `${team.stats.matchesPlayed > 0 ? (team.stats.wins / team.stats.matchesPlayed) * 100 : 0}%`}}></div>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-none shadow-lg mt-4">
              <CardContent className="p-6 text-center">
                ...
                <div className="space-y-3">
                  {team.captainEmail && (
                    <div className="flex items-center justify-center gap-3 p-2 rounded-lg hover:bg-gray-50 transition-colors"><Mail size={18} className="text-[#00a884]" /><span className="text-[#172c44]">{team.captainEmail}</span></div>
                  )}
                  {team.captainPhone && (
                    <div className="flex items-center justify-center gap-3 p-2 rounded-lg hover:bg-gray-50 transition-colors"><Phone size={18} className="text-[#00a884]" /><span className="text-[#172c44]">{team.captainPhone}</span></div>
                  )}
                </div>
              </CardContent>
            </Card>
