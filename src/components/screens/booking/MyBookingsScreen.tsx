...
// Íconos combinados de ambas versiones, asegurando que estén todos los necesarios (incluyendo X y ChevronDown)
...
import { toast } from 'sonner'; // Importación para manejar toasts
import { // Importación para manejar el diálogo de confirmación
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
  AlertDialogContent,
...
  // Se usa masterList y activities para la gestión de datos y filtros
...
  // Estados para el diálogo de confirmación (tomados de la segunda versión)
  const [showConfirmDialog, setShowConfirmDialog] = useState(false);
  const [activityToLeave, setActivityToLeave] = useState<DocumentData | null>(null);

  // Hook para cargar datos (combinando ambas lógicas de fetch y sort)
  useEffect(() => {
...
    fetchActivities();
  }, [auth.currentUser]); // Dependencia actualizada para mejor control

  // Hook para filtros (lógica de filtro combinada)
  useEffect(() => {
...
  const handleLeaveClick = (item: DocumentData) => {
    setActivityToLeave(item);
    setShowConfirmDialog(true);
  };

  // --- FUNCIÓN: SALIR O CANCELAR ACTIVIDAD (Ejecutada después de la confirmación) ---
  const handleConfirmLeave = async () => {
    if (!activityToLeave) return;
    const item = activityToLeave;
...
        // Eliminar de masterList (el filtro se encargará de activities)
...
        // Actualizar masterList para que refleje el cambio de status
...
      toast.error('Error', { description: err.message || "No se pudo completar la acción." });
    } finally {
      setLoading(false);
...
                // Permite navegar solo si no está cancelada
...
                      onClick={(e) => {
                        e.stopPropagation();
                        handleLeaveClick(item); // Usa la función con diálogo
                      }}
                    >
...
            );
          }
          return null; // Para asegurar que no renderiza nada si el tipo es desconocido
        })}
      </div>

      {/* --- DIÁLOGO DE CONFIRMACIÓN (Tomado de la segunda versión) --- */}
      <AlertDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              ¿Estás seguro de {activityToLeave?.type === 'match' ? 'salir del partido' : 'cancelar la reserva'}?
            </AlertDialogTitle>
            <AlertDialogDescription>
              {activityToLeave?.type === 'match' 
                ? `Saldrás del partido en ${activityToLeave.courtName}.` 
                : `La reserva de ${activityToLeave?.courtName} será marcada como cancelada.`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setActivityToLeave(null)}>Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={handleConfirmLeave} className="bg-red-600 hover:bg-red-700">
              {activityToLeave?.type === 'match' ? 'Sí, salir' : 'Sí, cancelar'}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
