...
import { 
  Calendar, Clock, DollarSign, Loader2, AlertTriangle, Inbox, 
  Search, Users, Shield, MapPin, LogOut, X, ChevronDown 
} from 'lucide-react'; 

// --- Importaciones de Componentes UI Propios y de 'ui' ---
...
export function MyBookingsScreen({ onBack, onNavigate }: MyBookingsScreenProps) {
  ...
  const handleConfirmLeave = async () => {
    ...
    try {
      if (item.type === 'match') {
        // --- LÓGICA PARA SALIR DE PARTIDO Y CHAT ---
        const matchRef = doc(db, "matches", item.id);
        const chatRef = doc(db, "chats", item.id);
        const playerName = currentUser.displayName || 'Un jugador';
        ...
        if (chatSnap.exists()) {
          await updateDoc(chatRef, {
            ...
            lastMessage: `${playerName} se ha salido del partido.`,
  ...
  // --- Funciones Auxiliares (Formatters) ---
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#172c44] to-[#00a884] pb-20">
      ...
      <div className="p-4 space-y-4">
        ...
        {/* --- LISTA DE ACTIVIDADES (LIMPIA Y ORDENADA) --- */}
        {!loading && activities.map((item) => {
          ...
          // --- TARJETA PARA PARTIDO AL QUE SE UNIÓ (LIMPIA) ---
          if (item.type === 'match') {
            return (
              <Card 
                key={item.id} 
      ...
      {/* --- DIÁLOGO DE CONFIRMACIÓN (LIMPIO) --- */}
