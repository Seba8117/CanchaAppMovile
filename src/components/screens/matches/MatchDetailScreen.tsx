...
// Revisa que estas rutas de UI sean correctas
import { Button } from '../../ui/button';
import { Card, CardContent } from '../../ui/card';
import { Badge } from '../../ui/badge';
import { Avatar, AvatarFallback } from '../../ui/avatar';

// Revisa que estas rutas de servicios sean correctas
import { getMatchById, deleteMatch } from '../../../services/matchService'; // Agregué deleteMatch
...
// Funciones utilitarias que faltaban y son necesarias
...
export function MatchDetailScreen({ match, onBack, onNavigate, userType }: MatchDetailScreenProps) {
    ...
    const [isJoining, setIsJoining] = useState(false); // No se usa, pero la mantengo
    const [joinError, setJoinError] = useState<string | null>(null);

    const currentUser = auth.currentUser;
    const isUserInMatch = currentUser && currentMatch.players?.includes(currentUser.uid); // No se usa
    const isMatchFull = currentMatch.currentPlayers >= currentMatch.maxPlayers; // No se usa
    ...
    // const [playersDetails, setPlayersDetails] = useState<any[]>([]); // Está duplicado con playerDetails


    useEffect(() => {
        // Carga inicial de detalles de jugadores y/o datos del partido
        const fetchData = async () => {
            setLoadingPlayers(true);
            setLoading(true); // Se inicia la carga general
            setError(null);
            
            const matchData = data || match; // Usar el dato más reciente
            const mustFetchPlayers = !!matchData?.players;

            try {
                // 1. Lógica para obtener los detalles de los jugadores
                ...
                // 2. Si el partido viene incompleto, obtener los datos frescos
                if (match?.id && (!matchData?.courtName || !matchData?.players)) {
                    const fresh = await getMatchById(match.id);
                    setCurrentMatch(fresh || match); // Actualiza el estado local del partido
                    setData(fresh || match);
                } else {
            ...
            } catch (error: any) { // ÚNICO BLOQUE CATCH
            ...
            } finally { // Bloque finally después del catch
        ...
        fetchData();
    }, [match?.id]); // Usamos solo match.id para evitar loops, la lista de players se actualiza internamente
    ...
    const teamIncluded = !!data?.teamId && !!data?.teamName; // No se usa, pero la mantengo
    ...
    const confirmDeleteMatch = async () => {
        try {
            // ASUMO que la función deleteMatch existe y se importó arriba.
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-[#172c44] to-[#00a884]">
            ...
            <div className="p-4 space-y-6 pb-24">
                ...
                <Card>
                    <CardContent className="p-6">
                        ...
                        {/* DETALLES DE HORA, FECHA Y JUGADORES */}
                        <div className="grid grid-cols-2 gap-4 mb-4">
                            <div className="flex items-center gap-2">
                                <Calendar className="text-[#172c44]" size={16} />
                                <span className="text-sm">{matchDate ? formatDate(matchDate) : 'Fecha no definida'}</span> 
                            </div>
                        ...
                        {/* Lógica de pago solo si es Owner */}
                        {isOwner && (
                            <div className="mt-3">
                ...
                <Card>
                    <CardContent className="p-4">
                        ...
                        <div className="space-y-3">
                            {loadingPlayers ? ( // Condición 1: ¿Está cargando?
                                <div className="flex justify-center p-4"><Loader2 className="animate-spin text-gray-500" /></div>
                            ) : playerDetails.length > 0 ? ( // Condición 2: Si no está cargando, ¿hay jugadores?
                                    ...
                                    <div key={player.id} className="flex items-center gap-3">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-xs text-gray-600">
                                                ...
                                                {player.id === data?.captainId ? 'Capitán' : 'Jugador'}
                                            </p>
                                        </div>
                                        <div className="flex items-center gap-1">
                                            <Star className="text-[#f4b400]" size={12} fill="currentColor" />
                                            <span className="text-xs text-gray-600">{player.rating?.toFixed(1) || 'N/A'}</span>
                                        </div>
                                    </div>
                                ))
                            ) : ( // Else final: No hay jugadores y no está cargando.
            ...
            {/* Fixed Bottom Button */}
            {/* El botón de Unirse (fijo) está duplicado, se deja solo el inferior para el Owner/Player */}
